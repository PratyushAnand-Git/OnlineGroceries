...
class HomeViewModel: ObservableObject {
    ...
    private func handleSuccessResponse(_ response: NSDictionary) {
        guard let payloadObj = response.value(forKey: KKey.payload) as? NSDictionary else { return }

        self.offerArr = (payloadObj.value(forKey: "offer_list") as? NSArray ?? []).compactMap {
            ProductModel(dict: $0 as? NSDictionary ?? [:])
        }
        self.bestArr = (payloadObj.value(forKey: "best_sell_list") as? NSArray ?? []).compactMap {
            ProductModel(dict: $0 as? NSDictionary ?? [:])
        }
        self.listArr = (payloadObj.value(forKey: "list") as? NSArray ?? []).compactMap {
            ProductModel(dict: $0 as? NSDictionary ?? [:])
        }
        self.typeArr = (payloadObj.value(forKey: "type_list") as? NSArray ?? []).compactMap {
            TypeModel(dict: $0 as? NSDictionary ?? [:])
        }
    }

    private func handleErrorResponse(_ response: NSDictionary) {
        self.errorMessage = response.value(forKey: KKey.message) as? String ?? "Fail"
        self.showError = true
    }
}
